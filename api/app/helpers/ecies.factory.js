...
import bufferSlice from 'buffer-slice';
...
export const encrypt =  (publicKeyTo, msg, opts) => {
  ...
  let ephemPublicKey;// = Buffer.concat([Buffer.from(ethUtil.toBuffer(ethUtil.addHexPrefix(publicKeyTo)), 'utf8')]);// = Buffer.from(publicKeyTo,'utf8');

  publicKeyTo = Buffer.concat([Buffer.from([0x04]),
    Buffer.from(ethUtil.toBuffer(ethUtil.addHexPrefix(publicKeyTo)), 'utf8')]);
